...
class EntityDisambiguator:
    ...
    def find_similar_predicates(
        ...
        for i, pred1 in enumerate(predicates_list):
            if pred1 in processed:
                continue

            similar_group = [pred1]
            info1 = predicates_with_info[pred1]
            processed.add(pred1)

            for j, pred2 in enumerate(predicates_list[i + 1 :], i + 1):
                if pred2 in processed:
                    continue

                info2 = predicates_with_info[pred2]
...
class ChunkRDFGraphAggregator:
    ...
    def aggregate_graphs(self, chunks: dict[str, Chunk]) -> RDFGraph:
        ...
        for group in similar_predicate_groups:
            ...
            local_name = best_pred.split("/")[-1].split("#")[-1]
            canonical_uri = URIRef(f"{self.doc_iri}/predicate/{local_name}")
