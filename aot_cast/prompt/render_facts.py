...
- for facts from the document, use <{current_doc_iri}> namespace with prefix `cd:` as `@prefix cd: {current_doc_iri} .`
- all entities identified by <{current_doc_iri}> namespace (facts, less abstract entities) must be linked to entities from either domain ontology <{ontology_iri}> or basic ontologies (RDFS, OWL etc), e.g. rdfs:Class, rdfs:subClassOf, rdf:Property, rdfs:domain, owl:Restriction, schema:Person, schema:Organization, etc 
- all facts should form a connect graph with respect to <{current_doc_iri}> namespace
